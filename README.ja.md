...
- **充実した診断** - SMP0001〜SMP0021（21 種）をコンパイル時に発行
...
| `[Mapper(Strict = true)]` | 未マップ destination プロパティを警告（SMP0016） |
...
> `init-only` / `record` destination に対して `void` マッパーは使用できません（SMP0018）。
...
> `Culture` なしで `DateTimeFormat` / `NumberFormat` のみ指定するとコンパイルエラー（SMP0019）。
...
> **`[MapExpression]` の注意** - 式の中にリフレクション API（`Activator`・`Type.GetType`・`MethodInfo` など）が含まれる場合、SMP0021 が発行されます。AOT 環境では `[MapFrom]` または `[MapUsing]` への置き換えを検討してください。
...
| コード | 説明 | 重大度 |
|--------|------|--------|
| SMP0001 | Mapper メソッドは `static partial` である必要があります | Error |
| SMP0002 | Mapper メソッドのパラメーター数が不正です | Error |
| SMP0003 | カスタムパラメーターに同じ型が複数指定されています | Error |
| SMP0004 | `BeforeMap` メソッドのシグネチャが一致しません | Error |
| SMP0005 | `AfterMap` メソッドのシグネチャが一致しません | Error |
| SMP0006 | Converter メソッドのシグネチャが一致しません | Error |
| SMP0007 | Converter メソッドの戻り値型がターゲットプロパティ型と一致しません | Error |
| SMP0008 | プロパティ条件メソッドのシグネチャが一致しません | Error |
| SMP0009 | `MapUsing` メソッドのシグネチャが一致しません | Error |
| SMP0010 | `MapUsing` メソッドの戻り値型がターゲットプロパティ型と一致しません | Error |
| SMP0011 | `MapFrom` で指定されたメソッドは引数なしのインスタンスメソッドである必要があります | Error |
| SMP0012 | `MapFrom` メソッドの戻り値型がターゲットプロパティ型と一致しません | Error |
| SMP0013 | `MapCollection` のマッパーメソッドが見つからないまたはシグネチャが一致しません | Error |
| SMP0014 | `MapNested` のマッパーメソッドが見つからないまたはシグネチャが一致しません | Error |
| SMP0015 | 同じ destination プロパティへの重複マッピングがあります | Error |
| SMP0016 | Strict モード: マップされていない destination プロパティがあります | Warning |
| SMP0017 | `required` メンバーが未マップです | Error |
| SMP0018 | `init-only` / `record` 型の destination に対して `void` マッパーは使用できません | Error |
| SMP0019 | `Culture` なしで `DateTimeFormat` / `NumberFormat` のみ指定されています | Error |
| SMP0020 | AOT 非互換: 汎用フォールバック `Convert<TSource,TDest>` に到達するマッピングがあります | Error |
| SMP0021 | AOT 警告: `MapExpression` 内に AOT 非互換の可能性があるリフレクションパターンが含まれています | Warning |

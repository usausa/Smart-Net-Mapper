...
| SMP0004 | `BeforeMap` メソッドのシグネチャが一致しない | エラー |
| SMP0005 | `AfterMap` メソッドのシグネチャが一致しない | エラー |
| SMP0006 | コンバーターメソッドのシグネチャが一致しない | エラー |
| SMP0007 | コンバーターの戻り値型が目的プロパティ型と一致しない | エラー |
| SMP0008 | プロパティ条件メソッドのシグネチャが一致しない | エラー |
| SMP0009 | `MapFrom` メソッドのシグネチャが一致しない | エラー |
| SMP0010 | `MapFrom` メソッドの戻り値型が目的プロパティ型と一致しない | エラー |
| SMP0011 | `MapFrom` ターゲットはソース型の引数なしインスタンスメソッドである必要がある | エラー |
| SMP0012 | `MapFrom` インスタンスメソッドの戻り値型が目的プロパティ型と一致しない | エラー |
| SMP0013 | `MapCollection` マッパーメソッドが見つからないまたはシグネチャが一致しない | エラー |
| SMP0014 | `MapNested` マッパーメソッドが見つからないまたはシグネチャが一致しない | エラー |
| SMP0015 | 同一目的プロパティへのマッピングが重複している | エラー |
| SMP0016 | Strict モード: マップされていない目的プロパティがある | 警告 |
| SMP0017 | `required` メンバーがマップされていない | エラー |
| SMP0018 | `init` 専用 / `record` 型の目的に `void` マッパーは使用不可 | エラー |
| SMP0019 | `Culture` なしで `DateTimeFormat` / `NumberFormat` を指定している | エラー |
| SMP0020 | AOT 非対応: 汎用 `Convert<TSource,TDest>` フォールバックに到達する可能性がある | エラー |
| SMP0021 | AOT 警告: `MapExpression` にリフレクションパターンが含まれる可能性がある | 警告 |
